...
HTTPS_GIT := https://github.com/kato114/byte.git
...
build-docker:
	...
	mkdir -p ./build/.byted
	echo '#!/usr/bin/env bash' > ./build/byted
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/byted
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/byted
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.byted:/home/evmos/.byted $$IMAGE_NAME byted "$$@"' >> ./build/byted
	chmod +x ./build/byted
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/kato114/byte"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evmos:Z evmos/node "./byted testnet init-files --v 4 -o /evmos --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
PACKAGE_NAME:=github.com/kato114/byte

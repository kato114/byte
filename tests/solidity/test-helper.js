...
function checkTestEnv() {
  const argv = yargs(hideBin(process.argv))
    ...
    .boolean("verbose-log")
    .describe("verbose-log", "print byted output, default false").argv;
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = "byted started";

    const bytedProc = spawn("../e2e/init-node.sh", {
      cwd: __dirname,
      stdio: ["ignore", "pipe", "pipe"],
    });

    logger.info(`Starting byted process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      bytedProc.stdout.pipe(process.stdout);
    }

    bytedProc.stdout.on("data", (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error("Start byted timeout!")), timeout);
  });
